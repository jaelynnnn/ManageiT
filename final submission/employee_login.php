...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        $sql = "SELECT * FROM staff WHERE staff_email = :email AND staff_key = :key";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':email', $staffEmail);
        $stmt->bindParam(':key', $staffKey);
        $stmt->execute();
        ...
        if ($staff) {
            ...
            $_SESSION['staff_key'] = $staff['staff_key'];
...
<div id="header">
    ...
    <div id="main">
        ...
        <form action="employee_login.php" method="POST">
            ...
            <input type="password" name="staff_key" required><br>
